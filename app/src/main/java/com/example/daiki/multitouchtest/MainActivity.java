package com.example.daiki.multitouchtest;
...
public class MainActivity extends Activity
    ...
    private class MakeRequestTask extends AsyncTask<Void, Void, List<String>> {
        ...
        void putDataToNewSheetFromApi() throws IOException {
            String spreadsheetId = "1tN_B8MrrjOjEUZFcgJDwuyuqMs9Tn97dScE2aASR9hw";
            ...
            if (intent.hasExtra("accel")) {
                ...
                float[] accelList = intent.getFloatArrayExtra("accel");
                for (int i = 0; i < accelList.length; i += 4) {
                    List col = new ArrayList<>();
                    col.add(accelList[i]);
                    col.add("ACCELEROMETER");
                    col.add(accelList[i + 1]);
                    col.add(accelList[i + 2]);
                    col.add(accelList[i + 3]);
                    row.add(col);
                }
                float[] gyroList = intent.getFloatArrayExtra("gyro");
                for (int i = 0; i < gyroList.length; i += 4) {
                    List col = new ArrayList<>();
                    col.add(gyroList[i]);
                    col.add("GYROSCOPE");
                    col.add(gyroList[i + 1]);
                    col.add(gyroList[i + 2]);
                    col.add(gyroList[i + 3]);
                    row.add(col);
                }
            } else {
